...
BEGIN
    ...
    DECLARE consulta_usloc VARCHAR(50);
    ...
    SELECT tipo_usuario_usloc INTO consulta_usLoc FROM UsLoc WHERE tipo_usuario_usloc = tipoUsuarioOfertado AND localizacion_localidad_usloc = localizacionLocalidad AND nombre_grada_usloc = nombreGrada AND nombre_recinto_usloc = nombreRecinto;
            ...
            IF recinto = nombreRecinto THEN
                IF consulta IS NOT NULL THEN
                    IF localidad = 'disponible' THEN
                        IF consulta_usLoc IS NOT NULL THEN
                            SET valido = TRUE;
                            LEAVE loop1;
                        ELSE 
